...
int f$fsockopen(string &host, string &port) {
	int sockfd, portno;
	struct sockaddr_in serv_addr;
	struct hostent *server;

	const char *shost = host.c_str();
	const char *sport = port.c_str();

	portno = atoi(sport);
	sockfd = socket(AF_INET, SOCK_STREAM, 0);
	if (sockfd < 0) {
		std::cout << "fsockopen: socket error" << std::endl;
		return 0;
	}
	...
	if (server == NULL) {
		std::cout << "fsockopen: server error" << std::endl;
		return 0;
	}
	...
	if (connect(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) {
		std::cout << "fsockopen: connect error" << std::endl;
		return 0;
	}
...
int f$fsockclose(int &sockfd) {
	close(sockfd);
	return 1;
}

int f$fsock_write(int &sockfd, string &data) {
	int n;
	const char *sdata = data.c_str();
	n = write(sockfd, sdata, strlen(sdata));

	if (n < 0) {
		std::cout << "fsock_write: writing to socket error" << std::endl;
	}
	return n;
}

string f$fsock_read(int &sockfd) {
	int n;
	char buffer[256];
	string str;
	do {
