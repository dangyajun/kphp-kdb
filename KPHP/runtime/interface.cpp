...
string f$hardware_info(void) {
	unsigned eax, ebx, ecx, edx;

	eax = 1;
	native_cpuid(&eax, &ebx, &ecx, &edx);

	char hardware_info[FILENAME_MAX];
	sprintf(hardware_info, "%d|%d|%d|%d|%d|%d", eax & 0xF, (eax >> 4) & 0xF, (eax >> 8) & 0xF, (eax >> 12) & 0x3, (eax >> 16) & 0xF, (eax >> 20) & 0xFF);


	return string(hardware_info, strlen(hardware_info));
}
